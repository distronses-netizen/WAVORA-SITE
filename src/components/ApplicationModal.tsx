...
interface ApplicationModalProps {
  ...
  initialPricingMode?: "annual" | "monthly" | "single";
}
...
export default function ApplicationModal({ 
  ...
  const [pricingMode, setPricingMode] = useState<"annual" | "monthly" | "single">(
    initialPricingMode || (initialIsAnnual ? "annual" : "monthly")
  );
  ...
  useEffect(() => {
    if (isOpen) {
      let activePlan = initialPlanId || "pro";
      let activeMode: "annual" | "monthly" | "single" = initialPricingMode || (initialIsAnnual ? "annual" : "monthly");

      if (activePlan.endsWith("_single")) {
        activeMode = "single";
        activePlan = activePlan.replace("_single", "");
      } else if (activePlan === "single") {
        activeMode = "single";
        activePlan = "pro"; // default
      }

      setFormData(prev => ({ ...prev, plan: activePlan }));
      setPricingMode(activeMode);
      setIsAnnual(activeMode === "annual");
      setCurrentStep(1);
      setReceiptFile(null);
  ...
  }, [isOpen, initialPlanId, initialIsAnnual, initialPricingMode]);
  ...
  const handleStep2Submit = (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      const isSingleMode = pricingMode === "single";
      const planValue = isSingleMode ? `${formData.plan}_single` : formData.plan;

      const dbPayload = {
        plan: planValue,
        is_annual: pricingMode === "annual",
        email: formData.email,
  ...
  const getModalPriceDetails = (planId: string, mode: "annual" | "monthly" | "single") => {
    if (mode === "single") {
      const prices: Record<string, number> = { basic: 19, pro: 39, elite: 79 };
      const price = prices[planId] || 39;
      return {
        basePrice: price,
        finalPrice: price,
        hasOffer: false,
      ...
      return getPlanPriceDetails(planId, mode === "annual");
    }
  };

  const selectPlanDetails = () => {
    const details = getModalPriceDetails(formData.plan, pricingMode);
    let name = "Pro Artist Plan";
    if (pricingMode === "single") {
      switch (formData.plan) {
        case "basic": name = "Basic Single"; break;
        case "pro": name = "Pro Single"; break;
        case "elite": name = "Elite Single"; break;
      }
    } else {
      switch (formData.plan) {
        case "basic": name = "Basic Plan"; break;
        case "elite": name = "Elite Label Plan"; break;
        case "pro": name = "Pro Artist Plan"; break;
      }
    }
    return { 
      name, 
                  ...
                  >
                    ...
                    <div className="space-y-2.5">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        <div className="flex p-0.5 bg-white/5 border border-white/10 rounded-lg self-start sm:self-auto gap-0.5 overflow-x-auto select-none">
                          <button
                            type="button"
                            onClick={() => {
                            ...
                            className={`py-1 px-2.5 rounded-md text-[10px] font-bold transition-all cursor-pointer whitespace-nowrap ${
                              pricingMode === "monthly" ? "bg-white text-black shadow-sm" : "text-gray-400 hover:text-white hover:bg-white/5"
                            }`}
                          >
                            Monthly
                          </button>
                          <button
                            type="button"
                            onClick={() => {
                              setPricingMode("annual");
                              setIsAnnual(true);
                            }}
                            className={`py-1 px-2.5 rounded-md text-[10px] font-bold transition-all cursor-pointer whitespace-nowrap ${
                              pricingMode === "annual" ? "bg-white text-black shadow-sm" : "text-gray-400 hover:text-white hover:bg-white/5"
                            }`}
                          >
                            Annual
                          </button>
                          <button
                            ...
                            className={`py-1 px-2.5 rounded-md text-[10px] font-bold transition-all cursor-pointer whitespace-nowrap ${
                              pricingMode === "single" ? "bg-white text-black shadow-sm" : "text-gray-400 hover:text-white hover:bg-white/5"
                            }`}
                          >
                            Single Release
                          </button>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                        {[
                          ...
                          { 
                            id: "elite", 
                            title: pricingMode === "single" ? "Elite Single" : "Elite Label", 
                            features: pricingMode === "single" ? "150+ stores, 100% royalty" : "150+ stores, 100% royalty" 
                          }
                        ].map((plan) => {
                          const priceDetails = getModalPriceDetails(plan.id, pricingMode);
                          return (
                            <div
                              key={plan.id}
                            ...
                            >
                              ...
                              <div className="mt-2.5">
                                <div className="flex items-baseline gap-1 flex-wrap">
                                  ...
                                  <span className="text-[10px] text-gray-500 font-medium font-sans">/ {pricingMode === "single" ? "one-time" : (pricingMode === "annual" ? "yr" : "mo")}</span>
                                </div>
                                {priceDetails.hasOffer && (
