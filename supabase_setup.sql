...
DROP TABLE IF EXISTS free_applications CASCADE;
...
-- ---------------------------------------------------------------------
-- 2.5 "free_applications" TABLE (Selective Free Distribution applications)
-- ---------------------------------------------------------------------
CREATE TABLE free_applications (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_free_applications_email ON free_applications(email);
CREATE INDEX idx_free_applications_status ON free_applications(status);
...
-- free_applications policies:
CREATE POLICY "Allow public all-around operations on free_applications" 
ON free_applications FOR ALL
USING (true)
WITH CHECK (true);
